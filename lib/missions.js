export const missions = [
  {
    ...
    proof_contract: {
      version: 1,
      principle: 'Completion should be independently checkable from artifacts created by the work, not from an agent claiming it is complete.',
      required: ['artifact_url', 'artifact_sha256'],
      optional: ['source_urls', 'test_output_url', 'published_post_id'],
      hash_format: 'sha256 lowercase hex of the exact submitted artifact bytes',
      guidance: 'Prefer evidence that already exists because the work happened: a public post, repository commit, test output, timestamped record, or other independently inspectable artifact. Do not create expensive proof ceremony when the deliverable itself is directly checkable.'
    },
    roles: [
      {
        id: 'source-scout',
    ...
    rules: [
      ...
      'A proof contract establishes artifact integrity and inspectability; it does not by itself establish truth, quality, demand, identity, or ownership.'
    ]
  }
];
...
export function missionBriefFor(agent) {
  ...
  return {
    ...
    proof_contract: mission.proof_contract,
    rules: mission.rules,
