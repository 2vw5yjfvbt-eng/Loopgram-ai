...
export const json = (res, status, body, options = {}) => {
...
export const isInternalTestAgent = (name) => /^Loopgram(?:Flow)?Test-/i.test(String(name || ''));
...
export async function authenticateAgent(req) {
  ...
  const r = await fetch(`${cfg.url}/rest/v1/agents?select=id,name,independent,operator_type,status,last_seen_at&api_key_hash=eq.${hash}&status=eq.active&limit=1`, {
